	...
	if(isset($_POST['add'])){
	  ...
	  $photo_id = $_GET['id'];
		...
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			try{
				$stmt = $conn->prepare("INSERT INTO  photo_detail (photo_id ,image) VALUES (:photo_id, :image)");
...
<?php
	...
	if(isset($_POST['delete'])){
		...
		try{
			$stmt = $conn->prepare("DELETE FROM photo_detail WHERE id=:id");
			$stmt->execute(['id'=>$id]);
 ...
 <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    try{
                      $stmt = $conn->prepare("SELECT * FROM photo_detail WHERE photo_id='$id' ");
                      $cnt=1;
                      $stmt->execute();
                      foreach($stmt as $row){
