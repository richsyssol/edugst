 ...
 <div class="row">
            <div class="col-12">
                <div class="card">
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-6">
                                    <div class="dt-buttons btn-group flex-wrap">
                                         <a href="#add-banner" class="btn btn-danger buttons-html5" data-bs-toggle="offcanvas"><i class="mdi mdi-plus-circle me-2"></i>Add Publication</a>
                                    </div>
                                </div>
                                <div class="col-sm-12 col-md-6">
                                 ...
                                 <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100 dataTable no-footer dtr-inline" role="grid" aria-describedby="datatable-buttons_info" style="width: 1244px;">
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <th class="sorting sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 199.4px;" aria-sort="ascending" aria-label="srno: activate to sort column descending">Academy</th>
                                                    <th class="sorting sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 199.4px;" aria-sort="ascending" aria-label="srno: activate to sort column descending">Date</th>
                                                    <th class="sorting sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 199.4px;" aria-sort="ascending" aria-label="Banner: activate">Thumbnail</th>
                    ...
                    try{
                      $stmt = $conn->prepare("SELECT * FROM tbl_publication");
                      $cnt=1;
                      $stmt->execute();
                      foreach($stmt as $row){
                      //  $image = (!empty($row['photo'])) ? 'assets/images/side_banner/'.$row['photo'] : 'assets/images/users/profile.jpg';
                        ...
                        <tr class="odd">
                            <td><?php echo htmlentities($cnt++);?></td>
                             ...
                             <td><a href="assets/images/publication/<?php echo $row['file'];?>" target="_blank"><i class="fa fa-file"></i> Uploaded File</a></td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="javascript:void(0);" data-bs-toggle="offcanvas" data-bs-target="#c_image" data-id="<?php echo $row['id'];?>" class="dropdown-item c_image"><i class="mdi mdi-plus-circle me-2 text-muted font-18 vertical-middle"></i></i>Edit</a>
                                        <a href="javascript:void(0);" data-bs-toggle="offcanvas" data-bs-target="#banner" data-id="<?php echo $row['id'];?>" class="dropdown-item banner"><i class="mdi mdi-plus-circle me-2 text-muted font-18 vertical-middle"></i></i>Update File</a>
           ...
           <?php 
            ...
            <?php include 'publication-popup.php'?>
...
function getRow(id){
  $.ajax({
    type: 'POST',
    url: 'publication_row.php',
    data: {id:id},
    dataType: 'json',
    success: function(response){
      $('.catid').val(response.id);
     $('#edit_name').val(response.name);
      $('$edit_subtitle').html(response.subtitle);
     $('$edit_academy').html(response.academy);
      $('$edit_date').html(response.date);
      $('$edit_url').html(response.url);
