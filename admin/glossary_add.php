...
<?php
	if(isset($_POST['add'])){
		...
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS total FROM tbl_glossary_list WHERE id=:id");
		$stmt->execute(['id'=>$id]);
		$row = $stmt->fetch();
		...
		//	$password = password_hash($password, PASSWORD_DEFAULT);
			...
			try{
			 $stmt = $conn->prepare("INSERT INTO tbl_glossary_list (key_id, title, description) VALUES (:key_id, :title, :description)");
				$stmt->execute(['key_id'=>$key_id, 'title'=>$title, 'description'=>$description]);
				$_SESSION['success'] = 'glossary added successfully';
...
<?php
	if(isset($_POST['delete'])){
		...
		try{
			$stmt = $conn->prepare("DELETE FROM tbl_glossary_list WHERE id=:id");
			$stmt->execute(['id'=>$id]);

			$_SESSION['success'] = 'Glossary deleted successfully';
		}
		catch(PDOException $e){
			$_SESSION['error'] = $e->getMessage();
		}
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box">
                            ...
                            <h4 class="page-title">Glossary List </h4>
                        </div>
                    </div>
                </div>  
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="text-uppercase bg-light p-2 mt-0 mb-3">Glossary List</h5>
                                <table  class="table table-hover small-text">
                                    ...
                                    $id=intval($_GET['id']);
                                    $query=mysqli_query($conn,"SELECT * from tbl_glossary_list where key_id='$id'");
                    ...
                    <div class="col-lg-6" >
                        <div class="card">
                            <div class="card-body">
                                <h5 class="text-uppercase mt-0 mb-3 bg-light p-2">Glossary Form</h5>
                                  <form  method="post" enctype="multipart/form-data">
                                       <input type="hidden" value="<?php echo $_GET['id'];?>" name="key_id">
                                    <div class="row">
                                         <div class="mb-3">
